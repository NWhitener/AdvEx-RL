...
class Navigation1(Env, utils.EzPickle):
    ...
    def step(self, a):
        ...
        #================================================
        #================================================
        # if not self.extended_abs:
        #     self.done = cur_cost > -4 or self.obstacle(next_state)
        #     if self.obstacle(next_state):
        #         adv_reward = float(self.obstacle(next_state))*float(-cur_cost)
        #         # adv_reward = float(self.obstacle(next_state))
        #     else:
        #         adv_reward = -0.01
        # else:
        #     if self.obstacle(next_state) and (not cur_cost > -4):
        #         if self.got_adv_reward:
        #             self.done = True
        #         else:
        #             self.done = False
        #     elif cur_cost > -4:
        ...
        #     if self.obstacle(next_state) and not self.got_adv_reward:
        #         adv_reward = float(self.obstacle(next_state))*float(-cur_cost)
        #         # adv_reward = float(self.obstacle(next_state))
        #         self.got_adv_reward = True

        #     elif self.obstacle(next_state) and self.got_adv_reward:
        #         adv_reward = float(self.obstacle(next_state))*float(-cur_cost)
        #         # adv_reward = float(self.obstacle(next_state))*float(-cur_cost)
        ...
        #         adv_reward = -0.01
        if self.obstacle(next_state):
            adv_reward = 1 #-cur_cost
        else:
            adv_reward = -0.0001 #-0.01
        #================================================
        #================================================
        #================================================
...
def render(loc, end=False):
    ...
    # circle = plt.Circle(loc, radius=1, color='green')
    # ax.add_patch(circle)
    # circle = plt.Circle((-30, 0), radius=1)
    # ax.add_patch(circle)
    # circle = plt.Circle((0, 0), radius=1)
    # ax.add_patch(circle)
    # label = ax.annotate("start", xy=(-30, 3), fontsize=10, ha="center")
    # label = ax.annotate("goal", xy=(0, 3), fontsize=10, ha="center")
